...
## Recent Changes
- 2026-04-27: Session-expiry UX fix. The dashboard used to fail silently when the admin session expired in the background (clicking "Save Product" did nothing visible because the request 401'd and the toast was generic). `csrfFetch` now (1) auto-retries once after refreshing the CSRF token if the server reports a CSRF mismatch (covers post-restart drift), and (2) on any 401 response, fires a single `alert("Your admin session has expired…")` and redirects to `/admin?expired=1`. The login page detects `?expired=1` and shows an amber inline notice ("Your session expired. Please sign in again to continue.") above the form. Net effect: after this change, the user will always know exactly why a save / upload didn't go through, instead of clicking a button and getting no feedback.
